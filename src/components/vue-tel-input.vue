<template>
  <div ref="refRoot" :class="['vue-tel-input', styleClasses, { disabled }]">
    <div v-click-outside="clickedOutside" aria-label="Country Code Selector" aria-haspopup="listbox"
      :aria-expanded="data.open" role="button"
      :class="['vti__dropdown', { open: data.open, disabled: dropdownOptions.disabled }]"
      :tabindex="dropdownOptions.tabindex" @keydown="keyboardNav" @click="toggleDropdown"
      @keydown.space="toggleDropdown" @keydown.esc="reset" @keydown.tab="reset">
      <span class="vti__selection">
        <span v-if="dropdownOptions.showFlags" :class="['vti__flag', toLowerCase(data.activeCountryCode)]"></span>
      ...
      <ul v-if="data.open" ref="refList" :class="['vti__dropdown-list', data.dropdownOpenDirection]" role="listbox">
        <div v-if="dropdownOptions.showSearchBox" class="vti__search_box_container">
          <slot name="search-icon"></slot>
          <input :class="['vti__input', 'vti__search_box']" aria-label="Search by country name or country code"
            :placeholder="dropdownOptions.searchBoxPlaceholder || (sortedCountries.length ? sortedCountries[0].name : '')"
            type="text" v-model="data.searchQuery" @click.stop />
        </div>
        <li v-for="(pb, index) in sortedCountries" role="option"
          :class="['vti__dropdown-item', getItemClass(index, pb.iso2)]"
          :key="pb.iso2 + (pb.preferred ? '-preferred' : '')" tabindex="-1" @click="choose(pb)"
          @mousemove="data.selectedIndex = index" :aria-selected="data.activeCountryCode === pb.iso2 && !pb.preferred">
          <span v-if="dropdownOptions.showFlags" :class="['vti__flag', toLowerCase(pb.iso2)]"></span>
          <strong>{{ pb.name }}</strong>
          <span v-if="dropdownOptions.showDialCodeInList"> +{{ pb.dialCode }} </span>
        </li>
      </ul>
    </div>
    <input v-model="data.phone" ref="refInput" :type="inputOptions.type" :autocomplete="inputOptions.autocomplete"
      :autofocus="inputOptions.autofocus" :class="['vti__input', 'vti__phone', inputOptions.styleClasses]"
      :disabled="disabled" :id="inputOptions.id" :maxlength="inputOptions.maxlength" :name="inputOptions.name"
      :placeholder="data.parsedPlaceholder" :readonly="inputOptions.readonly" :required="inputOptions.required"
      :tabindex="inputOptions.tabindex" :value="modelValue" :aria-describedby="inputOptions['aria-describedby']"
      inputmode="tel" @blur="onBlur" @focus="onFocus" @input="onInput" @keyup.enter="onEnter" @keyup.space="onSpace" />
